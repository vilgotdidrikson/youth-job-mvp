...
export default function VoiceCvPage() {
  ...
  return <main className="mobile-shell voice-cv-page" style={{ minHeight: "100svh" }}>
    ...
    <section className="card" style={{ display: "grid", gap: "1.1rem", padding: "1.35rem", textAlign: "center" }}>
      ...
      <button type="button" onClick={status === "idle" ? () => void startCall() : status === "recording" ? stopRecording : status === "paused" ? () => void resumeCall() : status === "complete" ? () => { stopCall(); const params = new URLSearchParams(window.location.search); const job = params.get("job"); const title = params.get("title"); router.push(`/youth/cv/create?voice=finalize${job ? `&job=${encodeURIComponent(job)}${title ? `&title=${encodeURIComponent(title)}` : ""}` : ""}`); } : undefined} disabled={status === "connecting" || status === "speaking" || status === "processing"} className="cta-btn" style={{ minHeight: "3.35rem", borderColor: VOICE_BUTTON_COLOR, background: VOICE_BUTTON_COLOR, fontSize: "1rem", opacity: status === "connecting" || status === "speaking" || status === "processing" ? .65 : 1 }}>{status === "connecting" ? "Preparing..." : status === "speaking" ? "Speaking..." : status === "recording" ? "Done speaking" : status === "processing" ? "Processing..." : status === "paused" ? (error ? "Try again" : "Continue") : status === "complete" ? "Continue" : "Start"}</button>
